...
namespace ORB_SLAM2
{
    ...
    Pri::Pri(const g2o::ellipsoid_zhjd &e) // 直接从椭球体读取 pri
    {
    ...
    g2o::ellipsoid_zhjd priorInfer::GenerateInitGuess(Vector4d& bbox, const Vector4d& plane_local)
    {
        std::cout<<"[GenerateInitGuess] 2 初始化一个椭球体"<<std::endl;
        std::cout << "bbox: " << bbox.transpose() << std::endl;
        std::cout << "plane_local: " << plane_local.transpose() << std::endl;

        // double dis_sigma = Config::ReadValue<double>("MonocularInfer.Init.Dis"); // 0.5m
        // double size_sigma = 0.1;

        ellipsoid_zhjd e_init_guess; // 获得估计出椭球体的 rpy; 只是将 x,y,z,a,b,c 都设置为0.
        Vector10d e_param;
        e_param <<  (   0, 0, 0,   // x y z
                        0, 0, 0, 0,  // qx qy qz qw
                        0.5, 0.5, 0.5   // length_a  length_b  length_c
                    );
        std::cout << "e_param: 1:" << e_param.transpose() << std::endl;
        // e_init_guess.fromVector(e_param);
        std::cout << "e_param: 6: 结束 e_init_guess"  << std::endl;
    ...
    //     // 还需要传入 bbox. x4
    //     int bbox_planes_num = e.mvCPlanes.size();  //ZHJD: 这些平面来自于GenerateConstrainPlanesOfBbox(...)函数
    //     std::vector<g2o::plane> planes_bbox; 
    //     planes_bbox.resize(bbox_planes_num);
    ...
    //     }
    //     // ZHJD:将边界框平面 planes_bbox 添加到 planes 向量中，以便在优化中使用。
