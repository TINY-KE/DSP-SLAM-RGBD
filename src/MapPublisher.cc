...
void MapPublisher::PublishObject(const vector<MapObject*> &vObjs ){
    ...
    for(size_t i=0; i< vObjs.size(); i++)
    {
        ...
        vector<float> color = colors_bgr[ (vObjs[i]->mnId +4) % 6];  //+1还是粉色/红色    +2绿色  +3蓝色  +4橙色   +5黄色
        ...
        {
            if(/*(vObjs[i]->mvpMapObjectMappoints.size() < 10) ||*/ (vObjs[i]->isBad() == true)   )
            {
                continue;
            }
        }
