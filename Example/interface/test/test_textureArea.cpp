...
// private:
    ...
    cv::Mat GenerateAreaMat(int rows, int cols, const g2o::SE3Quat& campose_wc, const Matrix3d& calib, EllipsoidSLAM::Map* pMap){
...
void highlightTextureAreaIn3D(cv::Mat& im, const EllipsoidTex& e, const Matrix3d& ellipseMat, const g2o::SE3Quat& camera_wc, const Matrix3d& calib, EllipsoidSLAM::Map* pMap)
{
    // 获得所有需要测试的二维点对应的三维点坐标
    // 1、从椭圆中获得区域
    clock_t time0_start = clock();
...
void test3DPoints(const EllipsoidTex& e, const Vector5d& ellipse, const g2o::SE3Quat& camera_wc, const Matrix3d& calib, EllipsoidSLAM::Map* pMap)
{
    // 测试中间点
    ...
    for(int i=0;i<test_mat_full.rows();i++){
        ...
        if(pMat.rows()>0)
        {
            ...
            EllipsoidSLAM::PointXYZRGB* pp = new EllipsoidSLAM::PointXYZRGB;
            EllipsoidSLAM::PointXYZRGB& p = *pp;
            p.x = point[0]; p.y = point[1]; p.z = point[2]; 
            p.r = 0; p.g=0; p.b =255; p.size = 10;
            pMap->addPoint(pp);

            point = pMat.row(1).head(3);
            EllipsoidSLAM::PointXYZRGB* pp2 = new EllipsoidSLAM::PointXYZRGB;
            EllipsoidSLAM::PointXYZRGB& p2 = *pp2;
...
int main(int argc,char* argv[])
{
    ...
    EllipsoidSLAM::System* pSLAM = new EllipsoidSLAM::System(path_setting, true);
    EllipsoidSLAM::Map* pMap = pSLAM->getMap();
