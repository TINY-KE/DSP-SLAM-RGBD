...
// est_id : 有效物体的序号
// ref_id : gt物体列表中的序号
std::map<int, int> AssociateFrame(Frame* pFrame, MatrixXd& refObjMat, EllipsoidSLAM::Map* pMap)
{
...
std::map<int, evo::COMPARE_RESULT> AnalyzeFrame(Frame* pFrame, MatrixXd& refObjMat, EllipsoidSLAM::Map* pMap)
{
    std::map<int, evo::COMPARE_RESULT> output_results;
...
std::map<int, std::map<double, int>> AssociateDetectionToGt(const std::vector<Frame*> &vpFrames, 
        MatrixXd& refObjMat, EllipsoidSLAM::Map* pMap)
{
    // 最后希望生成的 List 为:
    //  Gt -> { [timestamp, ob_id], ...   }
...
int main(int argc, char* argv[]) {
    ...
    EllipsoidSLAM::Map* pMap = mpSystem->getMap();
